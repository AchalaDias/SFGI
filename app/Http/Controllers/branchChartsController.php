...
use DB;
class branchChartsController extends Controller
{
    public function Branch_details(Request $request){
    	...
    	$key = $request->input("key");
            ...
            $Branch_result = DB::select(DB::raw("
 					select * from ge_sf_user_details where user_role = 'BRMGR' and branch='$key'
            "));
    ...
    public function chart1_0_1data_group_branch(Request $request){
    	 ...
    	 $key = $request->input("key");
            ...
            where W.business_party = 'AGENT' and W.assurance_code = '$key'   
            group by W.DEPT_NEW
            order by W.DEPT_NEW
    ...
    public function chart1data_group_branch(Request $request){
    	...
    	$key = $request->input("key");
          ...
          $chart_result = DB::select(DB::raw("
            ...
            where fmt.dept='MOTOR' and fmt.business_party = 'AGENT' and fmt.assurance_code = '$key'
    ...
    public function chart2data_group_branch(Request $request){
    	...
    	$key = $request->input("key");
            ...
            where  fmt.business_party = 'AGENT' and fmt.assurance_code ='$key' 
    ...
    public function cumulative_cwg_group_branch(Request $request){
 ...
 return json_encode($chart_result);
    ...
    public function chart4data_group_branch(Request $request){
        ...
        (
            ...
            from t_report_dncn_gwp_fmt fmt
            where fmt.dept='MOTOR' and  business_party = 'AGENT' and assurance_code = '$key'
            group by  fmt.pol_status
            ...
            where fmt.dept='MTR' and fmt.assurace_code = '$key'
            and year=extract(year from sysdate) and month<=extract(month from sysdate)                                                                             
    ...
    public function CMNOP_group_branch(Request $request){
    ...
    public function chart3data_group_branch(Request $request){
