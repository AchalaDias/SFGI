...
class RegionChartsController extends Controller
{

	  public function region_details(Request $request){

    	$key = $request->input("key");

            $Branch_result = DB::select(DB::raw("
 					select * from ge_sf_user_details where user_role = 'REMGR' and region='$key'
            "));
    ...
    public function chart1_0_1data_group_region(Request $request){
    	 ...
    	 $key = $request->input("key");
            ...
            where W.business_party = 'AGENT' and W.assurance_code in (select branch from ge_sf_user_details where region = '$key' )    
            group by W.DEPT_NEW
            order by W.DEPT_NEW
    ...
    public function chart1data_group_region(Request $request){
    	...
    	$key = $request->input("key");
          ...
          $chart_result = DB::select(DB::raw("
            ...
            where fmt.dept='MOTOR' and fmt.business_party = 'AGENT' and fmt.assurance_code in (select branch from ge_sf_user_details where region = '$key' ) 
    ...
    public function chart2data_group_region(Request $request){
    	...
    	$key = $request->input("key");
            ...
            where  fmt.business_party = 'AGENT' and fmt.assurance_code in (select branch from ge_sf_user_details where region = '$key' ) 
    ...
    public function cumulative_cwg_group_region(Request $request){
     ...
     $key = $request->input("key");
      ...
      $chart_result = DB::select(DB::raw("
            ...
            where assurace_code in (select branch from ge_sf_user_details where region = '$key' ) 
            ...
            'WCI','WCI',
            '','MISC') DEPT_NEW,T.* FROM t_report_dncn_gwp_fmt T) W          
            where assurance_code in (select branch from ge_sf_user_details where region = '$key' ) 
            group by W.DEPT_NEW
           )
 ...
 return json_encode($chart_result);
    ...
    public function chart4data_group_region(Request $request){
            ...
            from t_report_dncn_gwp_fmt fmt
            where fmt.dept='MOTOR' and  business_party = 'AGENT' and assurance_code in (select branch from ge_sf_user_details where region = '$key' ) 
            group by  fmt.pol_status
            ...
            where fmt.dept='MTR' and  business_party_new = 'AGENT' and fmt.assurace_code in (select branch from ge_sf_user_details where region = '$key' ) 
            and year=extract(year from sysdate) and month<=extract(month from sysdate)                                                                             
    ...
    public function CMNOP_group_region(Request $request){
    	  ...
    	  $key = $request->input("key");
            ...
            ,round(sum(fmt.gwp),2) GWP,count(fmt.dn_cn_no) nop
             ...
             where fmt.assurace_code in (select branch from ge_sf_user_details where region = '$key' )    
    ...
    public function chart3data_group_region(Request $request){
	  ...
	  $month = date('m');
        ...
        where t.branch in (select branch from ge_sf_user_details where region = '$key' ) 
